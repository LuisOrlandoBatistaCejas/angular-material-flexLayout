...
@Injectable()
export class ComprobanteDetailService implements OnInit {
  selectedComprobanteEventList = new EventEmitter<any>();
  comprobanteList: ComprobanteModel[] = [];
  comprobanteDetailList: ComprobanteDetailModel[] = [];
  constructor() {
    this.comprobanteDetailList.push(new ComprobanteDetailModel(663, '1', '1', 'lUIS', 'TRANSPORTE PASAJEROS TABABELA QUITO', 1.00000, 8.00000, 2.01000, 5.99000));
  ...
  ngOnInit() {
    this.selectedComprobanteEventList.subscribe(
      (list) => {
        this.comprobanteList = list;
      }
    );
  }
  getComprobantes() {
